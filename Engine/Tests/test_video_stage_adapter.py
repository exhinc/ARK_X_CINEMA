...
def test_video_assembly_is_resumable(tmp_path):
    ...
    def assemble(video, voice, destination):
        calls.append((video, voice))
        destination.write_bytes(b"final-mp4")
...
def test_missing_source_is_rejected(tmp_path):
    _complete_prerequisites(tmp_path)
    narration = tmp_path / "audio" / "narration.wav"
    with pytest.raises(VideoStageError):
        run_video_stage(tmp_path, "movie-001", tmp_path / "missing.mp4", narration, lambda *_: None)
