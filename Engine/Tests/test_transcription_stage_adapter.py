...
def test_ad_transcription_is_bound_to_transcription_stage(tmp_path, monkeypatch):
    root = tmp_path
    ad_audio = root / "source" / "movie_ad.mp3"
    output = root / "Projects" / "movie" / "transcripts" / "ad.srt"
    whisper = root / "bin" / "whisper-cli.exe"
    model = root / "models" / "model.bin"
    ad_audio.parent.mkdir(parents=True)
    whisper.parent.mkdir(parents=True)
    model.parent.mkdir(parents=True)
    ad_audio.write_bytes(b"audio")
    ...
    result = bind_ad_transcription(root, "movie", ad_audio, output, whisper, model)

    assert result.status == "complete"
    assert result.stage == "transcription"
    assert output.exists()
    assert (root / "state" / "movie" / "checkpoint.json").exists()


def test_completed_transcription_is_not_run_again(tmp_path, monkeypatch):
    root = tmp_path
    ad_audio = root / "ad.mp3"
    output = root / "Projects" / "movie" / "transcripts" / "ad.srt"
    whisper = root / "whisper.exe"
    model = root / "model.bin"
    for path in (ad_audio, whisper, model):
    ...
    monkeypatch.setattr("transcription_stage_adapter.transcribe_ad_to_srt", fake_transcribe)
    bind_ad_transcription(root, "movie", ad_audio, output, whisper, model)
    second = bind_ad_transcription(root, "movie", ad_audio, output, whisper, model)
