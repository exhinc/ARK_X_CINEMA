...
def run_intelligence_stage(root: Path, movie_id: str, timeline: dict[str, Any], model: str, *, base_url: str = "http://127.0.0.1:11434/api/generate", infer: Callable[..., OllamaResult] = infer_scene, max_chars_per_packet: int = 6000) -> list[dict[str, Any]]:
    ...
    def work() -> None:
        ...
        for packet in packets:
            ...
            results.append({"packet_id": packet["packet_id"], "scene_id": packet["scene_id"], "start": packet["start"], "end": packet["end"], "model": result.model, "duration_ms": result.duration_ms, "intelligence": result.parsed})
        destination.parent.mkdir(parents=True, exist_ok=True)
        destination.write_text(json.dumps(results, indent=2, ensure_ascii=False), encoding="utf-8")

    run_bound_stage(root, movie_id, StageBinding("intelligence", artifact.as_posix(), work))
