...
whisper.cpp into a timestamped SRT. This module only prepares/validates inputs;
it never treats a pre-existing AD SRT as a substitute for the AD audio path.
"""
...
def validate_srt(path: Path) -> list[str]:
    ...
    for number, block in enumerate(blocks, 1):
        ...
        if _timestamp_ms(start) >= _timestamp_ms(end):
            errors.append(f"Block {number}: start must be before end")
        if previous_end is not None and _timestamp_ms(start) < previous_end:
            errors.append(f"Block {number}: cue overlaps/reverses previous cue")
        previous_end = _timestamp_ms(end)
...
def find_ad_audio(files: Iterable[Path]) -> list[Path]:
    """Find likely external AD audio without accepting an AD SRT as audio input."""
    result: list[Path] = []
    for path in files:
        p = Path(path)
...
def transcribe_ad_to_srt(
    ...
    try:
        convert = subprocess.run(
            [ffmpeg_executable, "-y", "-i", str(ad_audio), "-ar", "16000", "-ac", "1", str(temp_wav)],
        ...
        # whisper.cpp CLI syntax used by the locked production path.
        result = subprocess.run(
            [str(whisper_executable), "-m", str(whisper_model), "-f", str(temp_wav), "-osrt", "-of", str(output_srt.with_suffix(""))],
            text=True, capture_output=True, encoding="utf-8", errors="replace", check=False,
        )
